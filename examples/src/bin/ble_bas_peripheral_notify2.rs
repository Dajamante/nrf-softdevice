...
use embassy_nrf::saadc::{AnyInput, Input as SaadcInput, Saadc};
...
async fn notify_adc_value<'a>(
    ...
    loop {
        ...
        // Hack to send the distance sensor data
...
async fn main(spawner: Spawner) {
    ...
    // Then we initialize the ADC. We are only using one channel in this example.
    let adc_pin = saadc::Input::degrade_saadc(p.P0_29);
    ...
    loop {
        ...
        let adc_fut = notify_adc_value(&mut saadc, &server, &conn, &mut trig, &mut echo);
